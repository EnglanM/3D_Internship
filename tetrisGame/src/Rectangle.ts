...
export class Rectangle {
    ...
    plane3:Mesh|null;
    plane4:Mesh|null;
   private stateHorizontal:boolean;
    ...
    create= () => {
       this.plane1= MeshBuilder.CreatePlane(`plane${this.number}`,{size:1});
        ...
        this.plane4.position=new Vector3(1.5,7.5,0);
    ...
    //method to rotate
    rotateToRight = () => {
        let x = this.plane2?.position?.x; // Note: Remove "!" here
        let y = this.plane2?.position?.y; // Note: Remove "!" here
        if (x !== undefined && y !== undefined) { // Check if x and y are defined
            if (this.stateHorizontal === true) {
                if (this.plane1) this.plane1.position = new Vector3(x, y + 1, 0); // Check if plane1 exists
                if (this.plane3) this.plane3.position = new Vector3(x, y - 1, 0); // Check if plane3 exists
                if (this.plane4) this.plane4.position = new Vector3(x, y - 2, 0); // Check if plane4 exists
                this.stateHorizontal = false;
            } else {
                if (this.plane1) this.plane1.position = new Vector3(x - 1, y, 0); // Check if plane1 exists
                if (this.plane3) this.plane3.position = new Vector3(x + 1, y, 0); // Check if plane3 exists
                if (this.plane4) this.plane4.position = new Vector3(x + 2, y, 0); // Check if plane4 exists
                this.stateHorizontal = true;
            }
        }
    }
    ...
    positionY= ()=> {
        let value= Math.min(this.plane1?.position.y!,this.plane2?.position.y!,this.plane3?.position.y!,this.plane4?.position.y!);
        return value;    
    }
    minPositionX= ()=> {
        let value =Math.min(this.plane1?.position.x!,this.plane2?.position.x!,this.plane3?.position.x!,this.plane4?.position.x!);
        
        return value;
    }
    maxPosiitonx= ()=> {
        let value= Math.max(this.plane1?.position.x!,this.plane2?.position.x!,this.plane3?.position.x!,this.plane4?.position.x!);
